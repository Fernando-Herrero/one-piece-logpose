...
export const Skeleton = ({
    ...
    const baseClasses = classnames("bg-secondary rounded", !noAnimation && "animate-pulse", className);
    ...
    // Para variant="custom", usar width y height personalizados
    if (variant === "custom") {
        const customStyle = {};
    ...
    // Para otras variantes, usar las clases predefinidas
    const skeletonClasses = classnames(
        baseClasses,
        variantClasses[variant]?.[size] || variantClasses.text.md
    );
...
export const SkeletonCard = ({
    ...
    return (
        <div className={classnames("p-4 border border-white rounded-lg bg-primary", className)} {...props}>
            {/* Header con avatar */}
            {showAvatar && (
